...
namespace osiris {
  int Executor::ic_iallu_fd_ = -1;
Executor::Executor(){
  ic_iallu_fd_ = open_ic_iallu_device();
  if (ic_iallu_fd_ < 0) {
    LOG_ERROR("Failed to open ic_iallu device. Aborting!");
    std::exit(1);
  }
...
Executor::~Executor()
{
  ...
  int Executor::open_ic_iallu_device() {
...
int Executor::TestTriggerSequence(const byte_array& trigger_sequence,
  ...
  }; ;// = CreateSequenceOfNOPs(trigger_sequence.size());
  ...
  for (int i = 0; i < no_testruns; i++) {
    ...
    //__sync_synchronize();
    //std::cout<<&cycles_elapsed_trigger<<std::endl;
    volatile int error = ExecuteTestrun(0, &cycles_elapsed_trigger);
    asm volatile ("dsb sy": : : "memory");  // 执行数据同步屏障
    asm volatile ("isb": : : "memory");     // 执行指令同步屏障
    asm volatile("dmb sy": : : "memory");
    asm volatile("nop": : : "memory");
...
//0x0000515000000580
  ...
  for (int i = 0; i < no_testruns; i++) {
    ...
    asm volatile("dmb sy": : : "memory");
    asm volatile("nop": : : "memory");
    volatile int error = ExecuteTestrun(1, &cycles_elapsed_notrigger);
    asm volatile ("dsb sy": : : "memory");  // 执行数据同步屏障
    asm volatile ("isb": : : "memory");     // 执行指令同步屏障
    asm volatile("dmb sy": : : "memory");
    asm volatile("nop": : : "memory");
...
///       for the signal handling + jmp logic to work
  ...
  if (!setjmp(fault_handler_jump_buf)) {
   ...
   // std::cout<<"in cycles_elapsed"<<*cycles_elapsed<<std::endl;
    ...
    sigset_t signal_set;
    sigemptyset(&signal_set);
    std::cout<<"error in execute codepage"<<std::endl;
    for (int sig : signals_to_handle) {
      sigaddset(&signal_set, sig);
    }
    sigprocmask(SIG_UNBLOCK, &signal_set, nullptr);
